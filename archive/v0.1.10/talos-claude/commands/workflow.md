---
description: Orquestra pipeline de desenvolvimento de feature no Talos (PRD → validação → entrevista → plano → execução → review)
argument-hint: <tool> <mode> <input-type> [input] [--interview] [--review] [--help]
---

Você está executando o comando `/workflow` do plugin **talos**.

Argumentos recebidos: `$ARGUMENTS`

## Ação

1. Invoque a skill **`talos`** passando os argumentos acima como input.
2. A skill é dona de toda a lógica: parsing de `<tool> <mode> <input-type> [input] [flags]`, orquestração das sub-skills, validação de ambiguidades, lógica de decisão A/B/C e formato de output. Siga o `SKILL.md` dela como contrato.
3. Se `$ARGUMENTS` estiver vazio ou contiver `--help`, mostre a sintaxe completa da skill e pare.

## Referência rápida de sintaxe

```
/workflow <tool> <mode> <input-type> [input] [flags]
```

- **tool**: `claude` · `cursor` · `codex`
- **mode**: `full` · `direct` · `interview-only`
- **input-type**: `backlog-item` · `idea` · `prd` · `brainstorm`
- **flags**: `--interview` · `--review` · `--help`

Exemplos:

```
/workflow <tool> full backlog-item "S05"
/workflow <tool> direct prd "/path/PRD_S05.md" --review
/workflow <tool> interview-only brainstorm "que tal dark mode?"
```

Não improvise comportamento fora do `SKILL.md`. Em caso de ambiguidade ou erro, siga as seções "Lógica de decisão" e "Error handling" da skill.

**Gates duros (v0.1.10):** o pipeline é orientado a artefato. Antes de iniciar, rode a **Fase 0 (Pré-flight)** — carregar config/defaults do pacote, resolver ids da família `<tool>` e garantir que cada sub-agent carregue o `SKILL.md` real antes de agir. Se uma skill exigida não existir como invocável neste host, **pare e reporte; nunca emule a skill inline**. Respeite os Gates G1–G11 da SKILL: artefato em disco antes de avançar (G1); em `full`, nenhum código antes de `PLAN_*.md` validado (G2); skills invocadas de verdade (G3); validador frio como sub-agent separado (G4); scan de ambiguidade determinístico com exclusões estreitas logadas (G5); status verificado contra disco (G6); plano/execução/review como sub-agents reais (G7); ordem validator→review, nunca paralelo (G8); orquestrador de mãos atadas e dispatch blocking (G9); roteamento por `<tool>`, família única e ids exatos, sem fallback cross-family (G10); em `full`, após `PLAN_*.md` validado, a próxima ação obrigatória é despachar `plan_execute` — nunca finalizar só com handoff (G11).
