...
Orquestra pipelines completos de desenvolvimento de features no projeto Talos, automatizando a sequência de skills (PRD generation → planejamento → execução → review) sob demanda.
...
Plugin usa `talos_workflows_config.md` para:
...
Veja `talos_workflows_config.md` para detalhes técnicos e mapeamentos completos.
...
- **Config/defaults no pacote:** `talos_workflows_config.md`, `defaults/paths.md` e `references/subagent_dispatch.md` viajam com o plugin; não exigem config na raiz do repositório usuário.
